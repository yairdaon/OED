...
\begin{abstract}
  ...
  case of iid measurement noise. We also spatially correlated
  measurement and model errors mitigate the clusterization
  phenomenon. As a part of the analysis we prove a Matrix Determinant
...
dimensions, with Gaussian prior and noise. In this model, maximizing
the expected information gain (KL divergence
...
infinite-dimensional Bayesian inverse problems. These includet tools
...
measurement error, so we observe $v(\x_j,T) = u(\x_j,T) + \eps(\x_j)$
...
dimensions\cite{Stuart10}. We denote by $\fwd$ the dynamics operator,
so that $u( \cdot,T) = \fwd u_0$, and the observation operator $\obs$
so that $u(\x_j,T) = (\obs u)_j, j=1,\dots,m$. The posterior
covariance is known and depends only on $\prcov, \fwd, \obs$ and
$\sigma^2$ (see section \ref{section:prelim} and \eqref{eq:postcov}
specifically).
...
( \hilo^* )^m$ (sometimes called a design). In an analogy with linear
...
and the prior are assumed independent of each other. The observation
and forward operators are merged $\tmp := \obs \fwd$. Since we need to
separate the observation operator $\obs$ from the forward operator
$\fwd$ we write this explicitly in \eqref{eq:inverse problem}. We now
turn to understand the terms present in \eqref{eq:inverse problem}.
...
measuring, e.g., the mean $\ell(u) = \int_{\Omega}u$ of a function is
not possible.
...
straightforward. From \eqref{eq:objective} and \eqref{eq:objective}:
\begin{align*}
  \begin{split}
...
\eqref{eq:constraints}. We then find their gradient in \eqref{eq:grad
  constraints}. These are use in the Lagrange multipliers
problem in section \ref{subsec:necessary}.
...
\eqref{eq:Lagrange mult1} we have:
\begin{equation*}
...
section \ref{subsec:characterization} and is summarized in Theorem
\ref{thm:char}.
...
\ref{lemma:twice woodbury} in the appendix justify the third (it is
trivial if $\fwd$ is invertible):
...
($\fwd\prcov\fwd^*$ is invertible, as mentioned in section
...
By definition of the objective \eqref{eq:objective}, \eqref{eq:pre
  manipulations} and Corollary \ref{cor:samve ev}:
...
for small measurement error levels, the clusterization effect will be
