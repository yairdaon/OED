...
%% \begin{restatable*}[Flat posterior covariance]{corollary}{flatpost}\label{cor:equal eigs}
%%  %\begin{corollary}\label{cor:eigenvalues}
%%   If $\obs$ is D-optimal and $k := \rank \obs$, then:
%%   \begin{enumerate}
%%     \item The covariance of the pushforwad $\fwd_{*} \post$ is $\left ( (\fwd
%%       \prcov \fwd^*)^{-1} + \sigma^{-2} \obs^*\obs \right )^{-1}$.
%%     \item The eigenvalues of said operator are
%%       \begin{equation}
%%         \theta_i =
%%         \begin{cases}
%%           \left(\frac{1}{k} \sum_{j=1}^k \lambda_j^{-1} + \sigma^{-2}\frac{m}{k} \right )^{-1} & i \leq k \\
%%           \lambda_i &  i > k 
%%         \end{cases}
%%       \end{equation}
%%     \item The eigenvectors of said operator are the eigenvectors of
%%       $\fwd \prcov \fwd^*$
%%   \end{enumerate}
%% \end{restatable*}%\end{corollary}

%% \begin{restatable*}[Flat posterior covariance]{corollary}{flatpost}\label{cor:equal eigs}
%%   Consider a D-optimal design $\obs$ for a Bayesian linear inverse
%%   problem where $\fwd$ is the forward operator and $\pr$ is the
%%   prior. Then the eigenvectors of the covariance of the pushforward of
%%   the posterior $\fwd_{*} \post$ equal the eigenvectors of the
%%   covariance of the pushforward of the prior. Moreover, the
%%   eigenvalues are constant for eigenvectors $1 \leq i \leq k$ where $k
%%   =\rank \obs$.
%% \end{restatable*}
