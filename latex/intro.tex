...
\subsection{A toy model: the 1D heat equation}
...
\item \label{q:generic} \textbf{Is measurement clusterization a
  ...
  these eigenvalues depends on how ill-posed the problem at hand is
  and does not depend much on the prior. see Theorem \ref{thm:char}
  and the Discussion for details. From a numerical experiments
  perspective, simulations of our model give rise to D-optimal designs
  that exhibit clusterization more than 95\% of the time (see Section
  \ref{subsec:lemma sims} for details). Given the genericity of our
...
Our results have several implications. Our answer to Question
\ref{q:generic} implies that encountering clusterization should be
expected in many different problems across many different scientific
fields. Thus, researchers that encounter clusterization should not be
surprised or wary. In Our answer to Question \ref{q:why}, we explain
what is the cause of clusterization. As it appears, the cause is
generic: a D-optimal design reduces uncertainty for a select set of
prior covariance eigenvectors --- those with the most prior
uncertainty, i.e.~those the practitioner cares about the most!
