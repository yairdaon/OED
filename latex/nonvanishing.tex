\section{Model Error Mitigates Measurement Clusterization}\label{section:non vanishing}
...
\begin{proof}
  ...
  \det A$. Statement and proof in Lemma \ref{lemma:MDL}.}:
  %
...
measurement clusterization are not D-optimal. Since $\tar$
\eqref{eq:objective} is not defined for $\sigma^2 = 0$ and identical
measurement vectors, we cannot make a statement regarding $\sigma^2 =
0$, except in the limiting sense described above. In conclusion, for
small observation error $\sigma^2$ levels, measurement clusterization
is mitigated by the presence of a non-zero model error $\modcov$ ---
answering Question \ref{q:mitigate} posed in the Introduction.
