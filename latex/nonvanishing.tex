...
\begin{proof}
  ...
  \end{equation}
  Next, we use an analogue of the matrix determinant lemma, whose
  statement and proof are recorded in the appendix:
  %
  \begin{align*}
    \tar( \obs ) - \tar( \obsm )
    %
...
small observation error levels, the sensor clusterization is mitigated
by the presence of a non-zero model error\footnote{Since the design
