...
\begin{proof}
  ...
  \end{equation}
  Next, use a generalization of the matrix determinant
  lemma\footnote{$\det(A + uv^*) = (1 + \langle A^{-1} u,u \rangle)
  \det A$.} to Hilbert spaces (Lemma \ref{lemma:MDL}). Precise
  statement and proof are given in the appendix:
  %
...
\begin{proof} \label{cor:same meas proof}
  ...
  enumerator and denominator (respectively) of \eqref{eq:conclusion}:
  %
  \begin{align*}
    \tar( \obs ) - \tar (\obsm ) &=
    \log \left ( 1 + \frac{
...
criterion is not defined for $\sigma^2 = 0$ and identical observation
vectors, we cannot make a statement regarding $\sigma^2 = 0$, except
in the limiting sense described above.}.
