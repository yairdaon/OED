...
The necessary first-order condition for D-optimality
\eqref{eq:conditions}, when $\Sigma= \sigma^2I$ (i.e. when $\modcov =
0$) becomes
...
problem. The following two lemmas allow us to gain better
understanding of the nonlinear eigenvalue problem.
...
\begin{proof}
  ...
  first statement in part (5) now follows from Lemma \ref{lemma:twice
...
\begin{figure}
  ...
  \caption{A comparison of the eigenvalues of the pushforward
    ...
    sub-optimal design (right) does not.
...
\meas_5 = \ev_1$), while the latter does not. Since both of these
designs are D-optimal, clusterization does not pose any obstruction to
D-optimality.

\begin{equation}\label{eq:two designs}
  \obs_1 =
  \begin{pmatrix}
    \sqrt{0.275}\ev_1 - \sqrt{0.725}\ev_2 \\
    \sqrt{0.275}\ev_1 + \sqrt{0.725}\ev_2 \\
    \ev_1 \\
    \ev_2 \\
    \ev_1 \\    
...
Supplementary material) generates $\obs_1$, which exhibits
clusterization. Modifying the numerical values of the prior does not
break the clusterization. Thus, the preference to clusterization of
Lemma \ref{lemma:free} is robust. The vast majority of randomized
...
clusterization is a generic property of the relaxed model presented in
this study.
...
\begin{figure}
  ...
  \caption{Clusterization (left, $\obs_1$ in \eqref{eq:two designs})
    ...
    --- their posteriors are equal. The left panel shows a D-optimal
    design with clusterization: $\meas_3 = \meas_5$. The right panel
    shows a D-optimal design without clusterization. The contribution
    to the posterior precision due to $\meas_1$ and $\meas_2$ is
    denoted $\meas_1\meas_2$. These contributions are plotted together
    because $\meas_1$ and $\meas_2$ are not in the direction of any
    eigenvector. Similarly for $\meas_3\meas_4$.}
