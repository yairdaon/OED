\section{D-Optimal Designs when $\modcov = 0$}\label{section:vanishing}
...
Theorem \ref{thm:char}. These conditions, however, still leave some
freedom in choosing $\obs$. This freedom may give rise to
...
\obs_2$ are presented in \eqref{eq:two designs}, and it is easy to
verify that $\obs_1^*\obs_1 = \obs_2^*\obs_2$. However, $\obs_1$
exhibits clusterization ($\meas_3 = \meas_5 = \ev_1$), while $\obs_2$
does not. Both of $\obs_1$ and $\obs_2$ are D-optimal, and we conclude
that clusterization poses no obstruction to D-optimality.
...
A computer implementation of Lemma \ref{thm:char} (code in
Supplementary material) generates $\obs_1$ as a solution to the
D-optimal design problem mentioned in the previous
paragraph. Randomized numerical simulations (see code in Supplementary
material) give rise to D-optimal designs that exhibit clustrization
more than 95\% of the time. We conclude that clusterization poses no
obstruction to D-optimality for the model presented in this study and
that sensor clusterization is generic.
