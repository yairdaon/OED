...
\begin{figure}
  ...
  \caption{A comparison of the eigenvalues of the pushforward
    ...
    accumulated height of $\sigma^{-2}m = 3$ in both plots. The
...
\section{Clusterization poses no obstruction to D-optimality}\label{section:clusterization}T
...
eigenvalues of $\fwd \prcov\fwd^*$ are $\lambda_1^{-1} = 0.1,
\lambda_2^{-1} = 0.2$, its eigenvectors are $\{\ev_i\}_{i=1}^\infty$
and $\sigma^2=1$. Consider the following $\obs_1, \obs_2$ (see also
Figure \ref{fig:clusterization}):
\begin{equation}\label{eq:two designs}
  ...
  \end{pmatrix}
\end{equation}
\noindent It is easy to verify that $\obs_1^*\obs_1 = \obs_2^*\obs_2$
and that both $\obs_1$ and $\obs_2$ are D-optimal. However, $\obs_1$
is a clustered design ($\meas_3 = \meas_5 = \ev_1$), while $\obs_2$ is
not.


We are now ready to answer some of the questions posed in the
introduction:

\textbf{Question \ref{q:why}}: Since $\obs_1$ is clustered and
D-optimal, measurement clusterization poses no obstruction to
D-optimality. Furthermore, a computer implementation of Lemma
\ref{thm:char} also generates $\obs_1$ as a solution to the D-optimal
...
\textbf{Question \ref{q:replace}}: Since $\obs_2$ is D-optimal but not
clustered, we conclude that it is possible to substitute a clustered
design for an equally optimal non-clustered design. Achieveing this in
the setup of our model not hard, but in real life problems this may
