...
%DIF ADD flat.tex   Wed Jul 31 13:34:55 2024
%DIF 2-3d2
%DIF < 
...
measuremnts require digging holes in the ground and a }\emph{\DIFadd{design}}
...
\DIFadd{Specifiyng the right }\DIFaddend set of measurements holds particular significance
...
}\DIFdelend \DIFaddbegin \DIFadd{are A- and D-optimality}\DIFaddend .
...
\end{equation*}
\DIFadd{We refer to a set of measuremnts that maximizes the D-optimality
...
\DIFadd{For concreteness, let us now }\DIFaddend consider a toy \DIFdelbegin \DIFdel{problem: inferring the initial condition of the }\DIFdelend \DIFaddbegin \DIFadd{model of a physical
process for which we wish to solve an inverse problem: a partial
differential equation known as the }\emph{\DIFadd{heat equation}} \DIFadd{in one
dimension (}\DIFaddend 1D \DIFaddbegin \DIFadd{heat equation henceforth). Readers less familiar with
...
temperature distribution the }\emph{\DIFadd{initial condition}}\DIFadd{. We wait for a
...
described by the following three equations:
  ...
  \end{alignat}
\end{subequations}
\DIFadd{As time passes, heat disspates across the rod as hotter regions,
...
\DIFadd{Solving the 1D heat equation is straightforward. Any initial heat
...
\DIFadd{The inverse problem of the 1D }\DIFaddend heat equation \DIFdelbegin \DIFdel{(details in the
supplementary material). In }\DIFdelend \DIFaddbegin \DIFadd{is difficult ("ill-posed")
...
(code generating these movies is located in module }\texttt{\DIFadd{movies.py}}
\DIFadd{in the accompanying
...
are well-behaved, see \mbox{%DIFAUXCMD
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.5\textwidth]{figs/dst_modelError0.png}
    \DIFaddendFL \caption{Measurement clusterization for \DIFdelbeginFL \DIFdelFL{optimal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{D-optimal }\DIFaddendFL designs \DIFdelbeginFL \DIFdelFL{when
      inverting }\DIFdelendFL for the
      \DIFdelbeginFL \DIFdelFL{initial condition }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inverse problem }\DIFaddendFL of the \DIFdelbeginFL \DIFdelFL{1D }\DIFdelendFL heat equation\DIFdelbeginFL \DIFdelFL{(see
      supplementary material for details)}\DIFdelendFL . Measurement locations were
...
relaxed model for \DIFaddbegin \DIFadd{an inverse problem where }\DIFaddend D-optimality \DIFdelbegin \DIFdel{that }\DIFdelend maintains
analytical tractability\DIFdelbegin \DIFdel{and enables the
identification of }\DIFdelend \DIFaddbegin \DIFadd{, where }\DIFaddend D-optimal designs \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{are identified via
...
%DIF < \subsection{An answer for Question \ref{q:generic}: Genericity of measurement clusterization}
  ...
  \DIFdelbegin \DIFdel{Randomized numerical
  }\DIFdelend \DIFaddbegin \DIFadd{We give two complementing answers to this question. First, from a
  ...
  experiments, where }\DIFaddend simulations of our model give rise to D-optimal
  designs that exhibit clusterization \DIFdelbegin \DIFdel{more than 95\% of the time (see
  code in supplementary material). Given our model's genericity}\DIFdelend \DIFaddbegin \DIFadd{with high probability in Section
  \ref{subsec:lemma_sims}. Thus, given the genericity of our model}\DIFaddend , we
  expect measurement clusterization to be a generic \DIFaddbegin \DIFadd{and ubiquitous
  }\DIFaddend phenomenon.
...
\item \label{q:why} \textbf{Why does measurement clusterization
  occur?} In Section \DIFdelbegin \DIFdel{\ref{section:why}, we present a compelling explanation for the optimality of clustered designs in the absence
  of model error}\DIFdelend \DIFaddbegin \DIFadd{\ref{subsec:why} (specifically, See
  ...
  relaxed model to the inverse problem of the 1D heat equation}\DIFaddend . Our
  analysis reveals that \DIFdelbegin \DIFdel{, in our model, }\DIFdelend a D-optimal design focuses on a select set of
  ...
  of locations where (a) \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{all }\DIFaddend relevant prior eigenvectors are
...
\DIFdelbegin \DIFdel{The cornerstone of our investigation into measurement clusterization is Theorem \ref{thm:char} proven in Section
\ref{section:vanishing}. The key insight of Theorem \ref{thm:char}
lies in its fifth component, which highlights that }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Implications}}
...
\ref{q:why} above): }\DIFaddend a D-optimal design \DIFdelbegin \DIFdel{aims to uniformly reduce posterior uncertainties for posterior
...
designs are best constructed in the space of observations. }\DIFdelend \DIFaddbegin \DIFadd{reduces uncertainty for a
...
compact operator.
Let \(\obs: \hilo \to \mathbb{R}^m\) a linear
measurement operator, where \(m \in \mathbb{N}\) is }\DIFdelend \DIFaddbegin \DIFadd{One interesting implication of the analysis presented here is that
clusterization can be expected once }\DIFaddend the number of measurements \DIFdelbegin \DIFdel{taken. Let \(\sigma^2 \in \mathbb{R}_{+}\) observation
noise variance,
\(\data = \obs \fwd \param + \eps\), where \(\eps \in
\mathbb{R}^m\) is iid \(\mathcal{N}(0, \sigma^2)\) noise.
Let \(\pr
...
\(\post\) the posterior (Gaussian) measure.
  }\DIFdelend \DIFaddbegin \DIFadd{exceeds
...
\DIFdelbegin %DIFDELCMD < \begin{theorem}\label{thm:char}
%DIFDELCMD <   %%%
\DIFdel{Let:
  }%DIFDELCMD < \begin{itemize}
%DIFDELCMD <     \item %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{It is important to note that we do not view measurement clusterization
...
%DIFDELCMD <   \item  %%%
\DIFdel{\(\tr{\obs^*\obs} = m\). }%DIFDELCMD < \item %%%
...
%DIFDELCMD <     \end{equation*}
%DIFDELCMD <   \item %%%
\DIFdel{The covariance of }\DIFdelend \DIFaddbegin \DIFadd{Lastly, we believe that when clusterization arises, it should serve as
a warning sign to practitioners. In the inverse problem of }\DIFaddend the \DIFdelbegin \DIFdel{pushforwad \(\fwd_{*} \post\) is \(\left
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{1D heat
...
\DIFaddbegin \DIFadd{This behavior stems from a well-known property of the heat equation:
...
best understood in the sapce of }\emph{\DIFadd{observations}}\DIFadd{, see Section
...
heat \DIFdelbegin \DIFdel{equation introduced in Section
...
\mbox{%DIFAUXCMD
\cite{Stuart10}}\hskip0pt%DIFAUXCMD
, and as such, \(\fwd \prcov \fwd^*\) is assumed
...
altogether).
}\DIFdelend \DIFaddbegin \DIFadd{equatin, the forward operator is determined by the time evolution
of the 1D heat equation \eqref{eq:heat1} and \eqref{eq:heat2}.
}

\DIFaddend Measurements are taken via a \emph{measurement operator} \(\obs\). It
...
and \(\obs\) are explicitly separated \DIFaddbegin \DIFadd{(}\DIFaddend as in
\cite{attia2022stochastic, cvetkovic2023choosing}\DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{). Note that }\DIFaddend \(\obs
\in ( \hilo^* )^m\), where \(m\) is the number of measurements
taken\DIFaddbegin \footnote{\DIFadd{The alert reader will likely ask how do we reconcile
...
the author}}\DIFaddend . Entries \(\meas_j, j=1,\dots,m\) of the \DIFdelbegin \DIFdel{observation }\DIFdelend \DIFaddbegin \DIFadd{measurement
}\DIFaddend operator \(\obs\) are called \emph{measurements}:

\DIFdelbegin %DIFDELCMD < \begin{equation*}%\label{eq:O}
%DIFDELCMD <   \obs u = (\meas_1(u), \dots, \meas_m(u) )^t \in \R^m,\ u \in \hilo.
%DIFDELCMD < \end{equation*}%%%
\DIFdelend \DIFaddbegin \begin{equation*}\obs u = (\meas_1(u), \dots, \meas_m(u) )^t \in \R^m,\ u \in \hilo.
...
the prior \DIFaddbegin \DIFadd{(see below) }\DIFaddend are assumed independent of each other. Thus,
...
covariance in \(\hilo\) \mbox{%DIFAUXCMD
\cite{Stuart10}}\hskip0pt%DIFAUXCMD
...
maximum for }\DIFdelend the D-optimality criterion \DIFdelbegin \DIFdel{does not exist, as it is not
bounded}\DIFdelend \DIFaddbegin \DIFadd{is unbounded}\DIFaddend . In contrast, in any real-life
problem where sensors are concerned, the norm of measurements recorded
by sensors is always one\DIFdelbegin \DIFdel{(of course, point evaluations are not in any Hilbert space of
functions we wish to consider)}\DIFdelend \DIFaddbegin \footnote{\DIFadd{Again, our analysis does not apply
...
$\tar$ is achieved by repeating a measurement\DIFdelbegin \DIFdel{, so designs that exhibit
measurement clusterization are not }\DIFdelend \DIFaddbegin \DIFadd{. Thus, for vanishing
noise levels, clustered designs cannot be }\DIFaddend D-optimal. \DIFdelbegin \DIFdel{Since $\tar$ is not defined for $\sigma^2 = 0$ and identical measurements, we cannot make
a
...
%DIFDELCMD < %%%
\DIFdelend \begin{lemma}[Simultaneous diagonizability]\label{lemma:sim_diag}
...
}\DIFdelend \DIFaddbegin \DIFadd{The proof of Lemma is also delegated to the Supplementary. It is
...
\DIFdel{Note that conjugating a matrix by $R^{(m)}$ changes only its $m$ and
  ...
  }%DIFDELCMD < \begin{equation}\label{eq:mm}
...
%DIF < % \item 
  ...
  \item
    \begin{equation*}
        ...
        \end{cases}.
    \end{equation*}
  \item The covariance of the pushforwad \DIFdelbegin \DIFdel{\(\fwd_{*} \post\) is \(\left
    ( (\fwd \prcov \fwd^*)^{-1} + \sigma^{-2} \obs^*\obs \right
    )^{-1}\) }\DIFdelend \DIFaddbegin \DIFadd{\(\fwd_{*} \postopt\) is \(\left
...
%DIFDELCMD < %%%
\DIFdel{According to Theorem \ref{thm:char} , }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Answer to Question \ref{q:why}}}\label{subsec:why}
\DIFadd{Theorem \ref{thm:char} helps us give a compelling explanation of the
...
We first utilize Theorem \ref{thm:char} to understand what a }\DIFaddend D-optimal
\DIFdelbegin \DIFdel{designs aim to capture
a small subset of eigenvectors of the prior covariance, specifically
the }\DIFdelend \DIFaddbegin \DIFadd{design will look like for our relaxed model; we then argue that
...
\DIFadd{First, recall the relevant parts of Theorem \ref{thm:char}. In our
relaxed model, for a D-optimal design $\opt$:
}\begin{itemize}
  \item \DIFadd{$\opt^*\opt$ and $\fwd\prcov \fwd^*$ are simultaneously
    diagonalizable.
  }\item \DIFadd{$\opt^*\opt$ has non-zero eigenvalues only for the $k:=\rank
    \opt^*\opt$ largest eigenvalues $\lambda_j$ of $\fwd\prcov\fwd^*$,
    i.e.~$\eta_j=0$ for $j>k$.
  }\item \DIFaddend $k$ \DIFdelbegin \DIFdel{eigenvectors with the highest prior variance.
Our model
naturally achieves this objective by not measuring eigenvectors $k+1$
and above, as proven in }\DIFdelend \DIFaddbegin \DIFadd{is monotonic in $m$, the number of measurements we take.
}\end{itemize}

\DIFadd{Consider $\fwd$ and $\prcov$ from the }\emph{\DIFadd{the inverse problem of the
heat equation}}\DIFadd{. As before, we denote the eigenvalues of
$\fwd\prcov\fwd^*$ by $\lambda_j$. We input these eigenvalues into our
}\emph{\DIFadd{relaxed}} \DIFadd{model, and find a D-optimal design $\opt$ for our
relaxed model, e.g.~by using Lemma \ref{lemma:free} in the proof of
}\DIFaddend Theorem \ref{thm:char}. \DIFdelbegin \DIFdel{Translating this
understanding to spatial problems, we anticipate that }\DIFdelend \DIFaddbegin \DIFadd{In our relaxed model, the measurements we take
are best utilized in reducing uncertainty for the first $k$
eigenvectors. So, }\DIFaddend a D-optimal design \DIFdelbegin \DIFdel{would favor measurement
locations where }\DIFdelend \DIFaddbegin \DIFadd{arising from our }\emph{\DIFadd{relaxed
model}} \DIFadd{completely avoids measuring eigenvectors $k+1$ and above.
}
...
preference, Fig.~\ref{fig:why} depicts the
scenario using the }\DIFdelend \DIFaddbegin \DIFadd{zero. Luckily, the eigenvalues of $\fwd\prcov\fwd^*$ decay very
quickly: for example, recall from Section \ref{subsec:toy} that the
eigenvalues of the forward model for the }\DIFaddend 1D heat equation \DIFdelbegin \DIFdel{with homogeneous Dirichlet boundary conditions (details
inthe supplementary material).The plot showcases four eigenvectors ,
scaled according to their prior standard deviations. Since eigenvectors beyond the fourth have insignificant prior eigenvalues, we exclude them from consideration. Notably, we observe that
...
there are only two locations where the third and fourth eigenvectorsapproach zero, while the
}\DIFdelend \DIFaddbegin \DIFadd{are $-(2\pi
n)^{-2}$. Thus a D-optimal design is a trade-off between measuring the
small number of eigenvectors we are interested in.
}

\DIFadd{We explore the abovementione trade-off for the inverse problem of the
...
present a compromise between the amplitude of the }\DIFaddend first and \DIFdelbegin \DIFdel{second eigenvectors exhibit
...
the pigeonhole principle}\DIFdelend \DIFaddbegin \DIFadd{second
...
\subsection{\DIFadd{Simulating Lemma \ref{lemma:free}}}\label{subsec:lemma_sims}
...
\item \DIFadd{Generate random diagonal $D\in \mathbb{R}^{k\times k}$ with
  ...
  $\ttr D = m$}\DIFaddend .
\DIFaddbegin \item \DIFadd{Conjugate $D$ by a random orthogonal matrix to form a positive
...
}\item \DIFadd{Since columns of $A$ correspond to measurement vectors, we call
...
\DIFaddbegin \DIFadd{Full results are located in the }\texttt{\DIFadd{simulations.csv}} \DIFadd{file within
...
}\texttt{\DIFadd{zeros.py}} \DIFadd{of said repository.
}
