...
%% We now turn our attention to maximizing $\bar{\tar}$ in $\eta$. The only
%% constraint on $\eta$ arises from the unit norm contraint on rows of
%% $\obs$, which implies that $\tr{\obs^*\obs} = \sum_i \eta_i= m$.
%% \begin{proposition}
%%   Fix $\lambda \in \mathbb{R}_+^k, \sigma^2 > 0$. Then
%%   $$
%%   \eta = \arg\max_{\sum_i \eta_i = m} \Phi(\eta; \lambda, \sigma^2)
%%   $$ 
%% \end{proposition}
...
%%   \begin{align*}
%%     \begin{split}
%%       \frac{d}{d\eta_i} \Phi(\eta; \lambda, \sigma^2) &= \frac{1}{\sigma^2\lambda_i^{-1} + \eta_i}, \\
%%       %
%%       %
%%       %
%%       \frac{d}{d\eta_i} (\sum_i \eta_i - m)  &= 1.
%%     \end{split}
%%   \end{align*}

%%   Denote the Lagrange multiplier $c$. Then
%%   \begin{align*}
%%     \begin{split}
%%       \sigma^2\lambda_i^{-1} + \eta_i &= \frac{1}{c}, \\
%%       %
%%       %
%%       %
%%       \sum_i \eta_i &= m.
...
%%   \begin{align*}
%%     \begin{split}
%%       \sigma^2\sum_{i=1}^k\lambda_i^{-1} + m &= kc^{-1} \\
%%       %
%%     \end{split}
%%   \end{align*}
...
%% Corollary \ref{cor:same ev} implies that instead of considering
%% $\obs$ when searching for a D-optimal design, we can restrict
%% ourselves to $k \leq m$ eigenvalues of $\obs^*\obs$ corresponding
%% to the first $k$ eigenvalues of $\fwd \prcov \fwd^*$.
...
\begin{figure}
  ...
  \caption{Characterizing a D-optimal design, compared to a
    sub-optimal design. Both designs are allowed three observations,
    ...
    not. In this example, a observation is taken partly in the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{lemma}[Increase due to a Observation]\label{lemma:design increase}
