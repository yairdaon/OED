...
\begin{proposition}\label{prop:optimizer}
  ...
  maximizes $\tar$ subject to $\sum_{i=1}^m \eta_i = m$ and $\rank
  \obs = k$.
\end{proposition}
\begin{proof}
  Below, we use a slight abuse of notation and cast the entire
  optimization problem of $\tar$ to the first $m$ eigenvectors of
  $\fwd\ \prcov \fwd^*$. We treat $\eta$ as if it is $m-$dimensional,
  since by \ref{}, $\eta_i = 0, i > m$. This abuse of notation gives
  us the freedom to think of the problem at hand in $m-$dimensions. We
  will now find the optimal $\eta_i$'s.

  First, let $A := \{i: \eta_i > 0\}$. A choice of $A$ where $\eta_i
  =0, \eta_j >0$ for some $1 \leq i < j \leq m$ is not optimal (just
  ...
  Summing, substituting $\sum_{i=1}^m \eta_i = \sum_{i=1}^k \eta_i =
  m$, and dividng by $k$:
  $$
  \frac{1}{2\alpha} = \frac{m}{k} + \frac{\sigma^2}{k} \sum_{j=1}^k \lambda_j^{-1}.
  ...
  \begin{align} \label{eq:etas}
    \begin{split}
      ...
      %
      %
      &=\frac{m}{k} + \frac{\sigma^2}{k} \sum_{j=1}^k \lambda_j^{-1} - \sigma^2 \lambda_i^{-1}.%\\
      %
      %
      %
  ...
  \end{align}
\end{proof}
An immediate corollary from Proposition \ref{prop:} and the formula
for posterior eigenvalues is:
\begin{corollary}
  If $\obs$ is optimal and $k := \rank \obs$, then:
  \begin{enumerate}
    \item The pushforwad/pullback (??) $\fwd \post$ covariance is
      $((\fwd \prcov \fwd^*)^{-1} + \sigma^{\pm 2} \obs^*\obs)$.
  ...
  \end{enumerate}
\end{corollary}
%% Thus, for a given $A$, we know which $\eta_i$ are optimal.

  %% Now, fix some $k$ and consider the constrained optimzation problem
  %% of maximizing $\tar$ subject to $\sum_{i=1}^k\eta_i=m$ (without the
  %% constraints $\eta_i \geq 0$). It is easy to verify via Lagrange
  %% multipliers that \eqref{eq:etas} are still optimal. Returning to our
  %% original constrained optimization problem, where $\eta_i \geq 0$,
  %% the optimal $\eta_i$'s are defined by \eqref{eq:etas}, where $k$
  %% takes the largest possible value for which \eqref{eq:etas} defines
  %% nonnegative $\eta_i$'s.

  %% By definition, $k$ satisfies $\eta_i > 0, i=1,\dots k$. Rearranging
  %% \eqref{eq:etas} for $\eta_i > 0$, we get that the following must
  %% hold:
  %% \begin{equation*} \label{eq:k}
  %% \lambda_i^{-1} < \sigma^{-2}\frac{m}{k} + \frac{1}{k}
  %% \sum_{j=1}^{k} \lambda_j^{-1}, i=1,\dots, k.
  %% \end{equation*}

  %% Since $\lambda_i$ are arranged in descending order, it suffices to
  %% verify that
  %% \begin{equation*}% \label{eq:k}
  %%   \lambda_k^{-1} < \sigma^{-2}\frac{m}{k} + \frac{1}{k} \sum_{j=1}^{k}
  %%   \lambda_j^{-1}.
  %% \end{equation*}
...
\begin{proof}
  ...
  (3) were proved in Corollary \ref{cor:true target}. Parts (4) and
  (5) were proved in \ref{prop:optimizer}. The only missing part is
  showing that we can indeed find $\obs$ such that $\obs^*\obs$ has
  the required eigenvalues. To this end, we utilize Lemma
...
\begin{corollary}
  ...
  \frac{1}{k} \sum_{j=1}^k \lambda_j^{-1}$. The posterior precision is
  constant for $1 \leq i \leq k$.
\end{corollary}
