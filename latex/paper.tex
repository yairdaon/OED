...
and forward operators are typically merged $\tmp := \obs \fwd$. Since
we need to separate the observation operator $\obs$ from the forward
operator $\fwd$ we write this explicitly in \eqref{eq:inverse
  problem}. We now turn to understand the terms present in
\eqref{eq:inverse problem}.
...
however, that $\postcov$ depends on $\obs$, and so
\eqref{eq:eigenproblem} is a nonlinear eigenvalue problem.
...
By definition of the objective \eqref{eq:objective}, \eqref{eq:pre
  manipulations} and Corollary \ref{cor:same ev}:
