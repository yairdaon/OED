...
%
\ydnote{I added figure \ref{fig:optimal vs not} prior to figure
  \ref{fig:clusterization}. I also added Theorem \ref{thm:char}, which
  neatly summarizes the results of section
  \ref{section:vanishing}. This should help understand section
  \ref{section:clusterization}, which was extended and modified
  heavily.}
\end{enumerate}
...
\item Page 4, Line 14: In equation (2.1), $\Sigma$ has not been
  ...
  \ydnote{Moved this equation to after $\Sigma$ is defined.}
...
\item Page 4, Line 21: The prior distribution $\mathcal{N}(0,
  ...
  %
  \ydnote{Great point. Assuming zero mean is hardly necessary, so I
...
\item Page 6, Line 7: Please describe what $\psi$ is in the
  ...
  \ydnote{Added comment before statment of theorem where $\tar$ first
    appears. I also added a definition of KL divergence in the
    beginning of section \ref{subsec:D optimal design}. I added
    Definition \ref{def:d optimality} which formally defines
    D-optimality.}
...
\item Page 6, Line 19-20: "In section 3.2 we relax the constraints on
  ...
  should be reworded.
  \ydnote{Rephrased and added an explanation.}
...
\ydnote{I was not able to arrive at the same results when the forward
  ...
  added. It is placed just before section
  \eqref{subsec:dynamics}. Specifically, we need it in (e.g.)
...
\ydnote{This is a great point! The flawed explanation was removed. I
  ...
  considering Banach spaces did not add much. The new derivations
  appear in \eqref{eq:pre manipulations} and \eqref{eq:mod
    objective}. Instead, I use the much more reaonable assumption that
  $\fwd \prcov \fwd^*$ is invertible.}
...
\ydnote{I added several plots that I believe make the arguments and
  conclusions clearer. I added Theorem \ref{thm:char}, summarizing the
  results for vanishing model error. I also added an illustration
  story with graduated lab cylinders, just before the statement of
  Theorem \ref{thm:char} (see also figure \ref{fig:cylinder}).}
...
\ydnote{The discussion on observation correlation and how it mitigates
  clusterization can be found in section \ref{section:non
    vanishing}. It is now stated clearly in the contributions
  section.}
...
etc.
%
\ydnote{All such bad references and links were fixed.}
...
%
\ydnote{I extended the discussion relating to clusterization. I added
  figure \ref{fig:optimal vs not} to illustrate how the posterior of a
  D-optimal design behaves. Then I extend the discussion to section
  \ref{section:clusterization}.}
...
required here.
%
\ydnote{Clarification was added at the end of the proof. ``There is
  nothing special about $t=0$'' means we could have chosen any other
  value $t_0$ instead.}
...
\ydnote{The expectation / integration over the prior space happens
  ...
  divergence to the beginning of section \ref{subsec:D optimal design}.}
...
not $\Psi$ ?  \ydnote{This line was unclear. I extended the
  ...
  $\obs^{\star}$ is the maximizer of $\tar(\obs)$. This is recorded
  explicitly in Definition \ref{def:d optimality}.}
  
Page 6, lines 19; the statement made about relaxing the constraints on
the observation operator is unclear.
%
\ydnote{This sentence was poorly written. It is now rephrased.}

Page 6, line 24, I believe the reference to (3.2) should be replaced
with (3.1).
%
\ydnote{I think the referee is mistaken. The gradient was indeed
  recorded in (3.2).}
...
\ydnote{This point is spot on. I added the definition of $T$ before
  the derivation, just after the statment of Lemma
  \ref{lemma:lax}. The proof referee 2 refers to is not the proof of
...
%
\ydnote{All comments on numbering and references are great and
  correct. All were fixed. I added an explanation of what
  $\widehat{\Gamma_{post}}$ means before Corollary \ref{cor:same
    meas}.}

Page 11, in the proof of Lemma A.1 (lines 39-43), the partial
derivative vanished from step 2 to step 3; please explain!
%
\ydnote{I added an extra equality line for explanation in the proof of
  lemma \ref{lemma:lax}.}
