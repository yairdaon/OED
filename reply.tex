...
\item Page 4, Line 21: The prior distribution $\mathcal{N}(0,
  ...
  \ydnote{It does not have to be, we can subtract the mean. An explanation was added}
...
\item Page 6, Line 7: Please describe what $\psi$ is in the
  ...
  function.
  \ydnote{Added comment before statment of theorem where
    $\tar$ first appears}
...
\item Page 6, Line 19-20: "In section 3.2 we relax the constraints on
  ...
  should be reworded.
  \ydnote{Added short explanation}
...
\item line 30: the author claims to present an analytically tractable
  ...
  here.
  \ydnote{Unfortunately, this work does not have
    immediate applications to (e.g.) spatial sensor placement
    problems. I offer a toy model for understanding sensor
    clusterization. I plan to use some of the conclusions in
    subsequent work but I find it out of scope for the work under
    consideration here.}
...
is not well-justified.  \ydnote{This is a great point! The flawed
  explanation was removed. I was able to remove that assumption. But
  it required further changes to the manuscript.}
...
made. \ydnote{Again, great point. $\fwd^{-*} = (\fwd^*)^{-1}$ but now
  I do not assume $\fwd$ is invertible so this was removed.}
...
uncorrelated with fixed variance $\sigma^2$.  \ydnote{The title and
  abstract were changed to reflect the fact that linear problems are
  concerned.  The treatment of correlated errors was emphasized.}
...
\ydnote{The discussion on observation correlation
  and how it mitigates clusterization can be found in the last
  section.}
...
the axes, and the plot-generation process.  \ydnote{I extended the
...
considered. \ydnote{Unfortunately, I am again not sure I understand
...
the posterior covariance matrix.  \ydnote{I think the referee implies
  that the D-optimal design is only as good as the assumptions it
  relies on. I could not agree more. However, as long as we do believe
  the assumptions made --- a D-optimal design is indeed optimal (in a
  precise and quantifiable way). I think the source of
  misunderstanding is that I used the term ``measurement'' where I
  should have used ``eigenvector''. I changed the paragraph
  accordingly. The transition from thinking of ``measurements'' to
  thinking of ``eigenvectors'' is confusing and I believe it is better
  explained in the updated version of the manusctipt.}
...
required here.  \ydnote{Clarification was added at the end of the
...
Page 2, line 11; wrong reference to Figure 1.3?  \ydnote{Reference now
  fixed.}
...
\ydnote{The expectation / integration over the prior space happens
  ...
  $$
  \mathbb{E}_{\data}[KL(\post^\data||\pr)] = \int KL(\post^\data||\pr) \der \mu(\data) = \int \int \log \frac{\der \post^\data}{\der \pr}(x) \der \post(x) \der \mu(\data)
...
the observation operator is unclear.  \ydnote{This sentence was poorly
  written. It is now rephrased.}

Page 6, line 24, I believe the reference to (3.2) should be replaced
with (3.1). \ydnote{This was actually OK. The gradient was recorded in
  (3.2).}
...
to 5, the first and third terms vanished; why?  \ydnote{This point is
  spot on. I added the definition of $T$ before the derivation. This
  is not the proof of Lemma 3.1 --- it is the first variation of the
  objective. I reworded a few sentences to make this clearer.}
