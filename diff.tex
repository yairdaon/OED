...
%DIF DEL paper.tex       Wed Sep 30 22:03:15 2020
%DIF ADD new_paper.tex   Wed Sep 30 23:03:56 2020
...
\begin{abstract}
  ...
  \DIFaddbegin \DIFadd{linear }\DIFaddend inverse problems. We suggest an analytically
...
posterior covariance matrix. In a frequentist \DIFdelbegin \DIFdel{settings}\DIFdelend \DIFaddbegin \DIFadd{setting}\DIFaddend , an optimal
...
\DIFaddend The theoretical foundations for inverse problems over function spaces
can be found in \cite{Stuart10}. \DIFdelbegin \DIFdel{This section is mostly devoted to
...
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{In this section we define notations that will be
  used throughout the paper.
%DIF > % A reader mostly familiar with the
...
%DIF > % \begin{itemize}
%DIF > %   \item Parameter space $\hil$.
%DIF > %   \item Data space $\ban$.
%DIF > %   \item Forward dynamics operator $\fwd: \hil \to \ban$.
%DIF > %   \item Observation operator $\obs: \ban \to \mathbb{R}^m$.
...
\subsection{Bayesian \DIFaddbegin \DIFadd{Linear }\DIFaddend Inverse Problems}\label{section:abstract OED}
...
prime example). \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We want to infer $\param \in \hil$, some parameter of
the dynamics given noisy observations of $\fwd \param$. We \DIFdelbegin \DIFdel{model $\param \sim
\normal(0,\prcov)$ }\DIFdelend \DIFaddbegin \DIFadd{take a zero
mean Gaussian prior $\pr$ for $\param$ (otherwise the mean can just be
subtracted everywhere). Hence, $\param \sim \pr = \normal(0,\prcov)$
}\DIFaddend with some appropriate covariance operator $\prcov$
...
it is useful to \DIFdelbegin \DIFdel{rewrite }\DIFdelend \DIFaddbegin \DIFadd{record a known result regarding }\DIFaddend the posterior
covariance operator of our inverse problem\DIFdelbegin \DIFdel{from }%DIFDELCMD < \eqref{eq:postcov precursor}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, namely
}\DIFaddend \begin{align}\label{eq:postcov}
  \postcov = (\prcov^{-1} + \fwd^* \obs^* \Sigma^{-1} \obs \fwd
  )^{-1}.
\end{align}
\DIFaddbegin \DIFadd{Usually, this is written using $\tmp := \obs \fwd$ and $\tmp^*$ but
since we need to separate the observation operator $\obs$ from the
forward operator $\fwd$ we write the explicit expression
}\eqref{eq:postcov}\DIFadd{.
}\DIFaddend 
...
the expected KL divergence between posterior and prior \DIFaddbegin \DIFadd{(denoted $\tar$
below and throughout the paper) }\DIFaddend and is summarized, using our notation,
...
\DIFaddend From \eqref{eq:objective} we wish to characterize solution(s) of the
...
(e.g.\ pointwise evaluations on $[0,\pi]$)\DIFaddbegin \DIFadd{.
}\DIFaddend %%, for the example in section \ref{subsec:example}).
...
of $\tar$ \DIFaddbegin \DIFadd{and record it in }\eqref{eq:tar grad}\DIFaddend . In section
\ref{subsec:unit length} we relax the constraints on $\obs$ so that
the optimization problem in \eqref{eq:optimization} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{is analytically
tractable.  We }\DIFaddend find gradients for the new constraints \DIFaddbegin \DIFadd{and record them
in }\eqref{eq:grad constraints}\DIFadd{. Finally, in section
\ref{subsec:necessary} we use these to formulate the optimal design
problem as a Lagrange multipliers problem in equation
}\eqref{eq:conditions}\DIFaddend .
...
\DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{lemma}[Generalized from \cite{Lax97}]%%%
\DIFdelend \DIFaddbegin \DIFadd{\ref{lemma:lax}:
}\begin{restatable*}[Generalized from \cite{Lax97}]{lemma}{lax}\DIFaddend \label{lemma:lax}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{restatable*}
\DIFaddend 
...
\DIFdelend \DIFaddbegin \begin{restatable*}{proposition}{biggerbetter}\DIFaddend \label{prop:bigger better}
...
}\eqref{eq:mod conditions}\DIFadd{. Next, we characterize optimal designs as
...
\subsection{\DIFadd{The Eigenvalue Problem}}\label{subsec:eigenproblem}
\DIFaddend When $\modcov = 0$, then $\Sigma= \sigma^2I$. The necessary
...
of $\obs$, namely $\meas_j,j=1,\dots, m$, are the eigenvectors. \DIFaddbegin \DIFadd{Below,
we use the expression for the posterior covariance }\eqref{eq:postcov}
\DIFadd{in the second equality. }\DIFaddend If $\fwd$ is invertible, \DIFdelbegin \DIFdel{then we may write:
...
%DIFDELCMD < \end{align}  
%DIFDELCMD < %%%
\DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{the third equality is
...
\ref{lemma:twice woodbury} in the appendix:
  ...
  \end{split}
\end{align}  
\DIFadd{This }\DIFaddend presents the optimal design problem as a problem on $\ban$ and
not on $\hil$ --- the term $(\fwd \prcov \fwd^*)^{-1}$ is the prior
precision in $\ban$ and \DIFdelbegin \DIFdel{the big parentheses enclose }\DIFdelend \DIFaddbegin \DIFadd{$(\fwd\prcov\fwd^*)^{-1} + \sigma^{-2} \obs^*
\obs$ is }\DIFaddend the posterior precision in $\ban$.
\DIFdelbegin \DIFdel{This expression makes sense even if $\fwd$ is not
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{subsec:characterization}
\DIFaddend In light of \eqref{eq:mod conditions}, let us denote eigenvalues of
$\fwd \prcov \fwd^*$ by $\lambda_1 \geq \lambda_2 \geq \dots$ and its
eigenvectors $\{\ev_i\}_{i=1}^{\infty}$. The following lemma \DIFdelbegin \DIFdel{will be useful}\DIFdelend \DIFaddbegin \DIFadd{(proved
...
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{restatable*}{lemma}{simdiag}\label{lemma:sim diag}
  \DIFaddend Let $C:\hil \to \hil$ self-adjoint and let $\func_1,\dots,\func_m \in
...
\geq k+1$. \DIFdelbegin \DIFdel{By the argument in the paragraph following }%DIFDELCMD < \eqref{eq:mod
...
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Using Sylvester's determinant theorem we get:
}
...
%DIFDELCMD < \end{align}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{align}\label{eq:mod objective}
  \begin{split}
  ...
  \end{split}
\end{align}
\DIFaddend Therefore, an optimal design maximizes
...
in \eqref{eq:eigenproblem} are all equal \DIFdelbegin \DIFdel{! }\DIFdelend This makes perfect
...
discussion in the following paragraph).\DIFdelbegin \DIFdel{We are also used to symmetric
...
in this context.
}\DIFdelend %DIF > %  We are also used to symmetric
%DIF > % optimization problems having symmetric solutions \cite{Waterhouse83},
%DIF > % so symmetric and identical Lagrange multipliers definitely make sense
%DIF > % in this context.
...
%DIFDELCMD <   %%%
\DIFdel{If $\meas_m = \meas_j$ for some $1 \leq j \leq m-1$, then
  }\begin{displaymath}
    \DIFdel{\tar(\obs) - \tar(\obsm) =
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Results of section \ref{subsec:characterization} specifically, the
discussion at the end of said section, along with figure
\ref{figure:clusterization} and equation }\eqref{eq:true target} \DIFadd{show
...
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \section{\DIFadd{Analysis of Optimal Designs --- Non-Vanishing Model Error}}
\subsection{\DIFadd{Overview}}
\DIFadd{In this section we show the effect model error has on the
clusterization phenomenon. }\DIFaddend In contrast to the previous case of
vanishing model error, we will see that if $\modcov \neq 0$ clustering
will not occur.
\DIFaddbegin 

\subsection{\DIFadd{Derivation}}
\DIFadd{We rely on corollary \ref{cor:same meas} below, which is proved in the
...
(\meas_1,\dots,\meas_{m-1})^t$.
  ...
  \DIFadd{If $\meas_m = \meas_j$ for some $1 \leq j \leq m-1$, then
  }\begin{equation*}
    \DIFadd{\tar(\obs) - \tar(\obsm) =
    \log \left ( 1 + \frac{\sigma^2
      \langle \fwd \postcovm \fwd^* \obsm^* \Sigmam^{-1} e_j,
      \obsm^* \Sigmam^{-1}e_j \rangle
    }{
...
\end{restatable*}
\DIFaddend From Corollary \ref{cor:same meas} we can \DIFdelbegin \DIFdel{observe that
as $\sigma^2 \to 0$, the
}\DIFdelend \DIFaddbegin \DIFadd{deduce that
}

$$
\DIFadd{\lim_{\sigma^2 \to 0} \tar(\obs) -\tar(\obsm) = 0
}$$
\DIFadd{Hence no }\DIFaddend increase in the design criterion \DIFdelbegin \DIFdel{gained by taking a measurement
identical to a previous one approaches zero}\DIFdelend \DIFaddbegin \DIFadd{is
achieved. This is clearly sub-optimal and $\obsm$ cannot be an optimal
design}\DIFaddend . We may conclude that for small measurement error levels, the
...
\DIFdel{Let $Y(t)$ be a differentiable operator-valued function. Assume 
  $I+Y(t)$ is invertible, $Y(t)$ self-adjoint and trace-class. Then
    ...
    \DIFdel{\frac{\der \log \det (I+Y(t))}{\der t} = \tr{ (I+Y(t))^{-1} \dot{Y}(t)}.
  }\end{displaymath}%DIFAUXCMD
%DIFDELCMD < \end{lemma}
%DIFDELCMD < %%%
...
\DIFdelend \DIFaddbegin \begin{align*}
    ...
    = \tr{\dot{X}(0)},
  \end{align*}
  \DIFaddend where the second equality follows by monotone convergence. 
...
\DIFdelend \DIFaddbegin \begin{align*}
    ...
    \leq \tr{I+Y(t) - I}< \infty,
  \end{align*}
  \DIFaddend since $Y(t)$ is trace-class. It is also clear that
...
\DIFdelend \DIFaddbegin \begin{align*}
  ...
  \end{align*}
  \DIFaddend Consequently, by the one-variable chain rule:
  \DIFdelbegin %DIFDELCMD < \begin{align*}
...
\DIFdelend \DIFaddbegin \begin{align*}
  ...
  \end{align*}
  \DIFaddend Since there is nothing special about $t=0$, the relation holds for all $t$.
\end{proof}
...
\begin{lemma}[Matrix Determinant Lemma in Hilbert Spaces]\label{lemma:MDL}
  ...
  \hil$ an invertible linear operator such that \DIFdelbegin \DIFdel{$\tr A-I <
  \infty$}\DIFdelend \DIFaddbegin \DIFadd{$\tr{A-I} <
  \infty$}\DIFaddend . Then $\det A$ and $\det A + uv^*$ are well defined and
  \begin{equation*}
...
\begin{proof}
  ...
  It is easy to verify that $uv^*$ is trace-class and since \DIFdelbegin \DIFdel{$\tr A - I
  < \infty$, also $\tr A + uv^* - I < \infty$ }\DIFdelend \DIFaddbegin \DIFadd{$\tr{A-I}
  < \infty$, also $\tr{A + uv^* - I} < \infty$ }\DIFaddend (sum of two trace-class
...
\DIFaddbegin \begin{lemma}\label{lemma:twice woodbury}
  \DIFadd{Assume $\fwd \prcov \fwd^*$ is invertible. Then
}\begin{align*}
  \begin{split}
...
\end{lemma}
\begin{proof}
  \DIFadd{The proof is boring and amounts to using Woodbury's matrix identity
  ...
  \DIFadd{(A + UCV)^{-1} = A^{-1} + A^{-1}U(C^{-1} + VA^{-1}U)^{-1}VA^{-1}.
  }$$

  \DIFadd{Taking $A := \prcov^{-1}, U := \fwd^*, V := \fwd$ and $C :=
  \sigma^{-2} (\obs^*\obs+\eps I)$ we get:
    ...
    \end{split}
  \end{align*}
  \DIFadd{Note that $X + \sigma^2 Y_{\eps}^{-1}$ is invertible, as the sum of
  ...
  \DIFadd{We conclude that $\forall \eps > 0$
  }\begin{align*}
    \begin{split}
      \fwd( \prcov^{-1} + \sigma^{-2}  \fwd^* (\obs^* \obs +\eps I) \fwd )^{-1}\fwd^* 
     &= (\fwd \prcov \fwd^* + \sigma^{-2} (\obs^*\obs + \eps I))^{-1}.
    \end{split}
  \end{align*}
  \DIFadd{Letting $\eps \to 0$ completes the proof.
}\end{proof}
...
\DIFaddend \begin{proof} \DIFaddbegin \label{cor:same meas proof}
  \DIFaddend Denote $A:= \obs \modcov \obs^*$ and $v_j$ the $j$th column of $A$.
...
\begin{lemma}\label{lemma:free}
  Let $M \in \R^{k \times k}$ symmetric positive definite with \DIFdelbegin \DIFdel{$\tr M =
  m$}\DIFdelend \DIFaddbegin \DIFadd{$\tr{M} =
  m$}\DIFaddend , $m > k$. Then we can find $\func_j \in \R^k,j=1,\dots,m$ with
...
\begin{proof}
  ...
  \end{equation}
  and we can expect to do this since we assumed \DIFdelbegin \DIFdel{$\tr D = m$}\DIFdelend \DIFaddbegin \DIFadd{$\tr{D} = m$}\DIFaddend .

  Define $C = S^tS - I \in \R^{m \times m}$. Note that \DIFdelbegin \DIFdel{$\tr C = 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\tr{C} = 0$ }\DIFaddend and
  ...
  continue introducing zeros to the diagonal. The assumption \DIFdelbegin \DIFdel{$\tr D =
  m \Rightarrow \tr C = 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\tr{D} =
  m \Rightarrow \tr{C} = 0$ }\DIFaddend guarantees we can do that. Taking $V:=
...
%DIFDELCMD <   %%%
\DIFdel{Let $C:\hil \to \hil$ self-adjoint and let $\func_1,\dots,\func_m \in
  \hil$. Denote $\func^*$ the element $\func$ acting as a
  functional. If
  }\begin{displaymath}
   \DIFdel{(C + \sum_{j=1}^m \func_j\func_j^*) \func_k = \xi_k \func_k, k = 1,\dots,m
  }\end{displaymath}%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{then $C$ and $\sum_{j=1}^m \func_j \func_j^*$ are simultaneously
  diagonalizable.
}%DIFDELCMD < \end{lemma}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \simdiag
\DIFaddend \begin{proof}
...
%DIFDELCMD <   %%%
\DIFdel{Let $\obs = (\meas_1,\dots,\meas_m)^t$, $j \in \{1,\dots,m\}$, $\sigma^2
  > 0$ and $\lambda > 1$. Then $\tar(\obs)$ increases if we use
  $\lambda \meas_j$ in $\obs$ instead of $\meas_j$.
}%DIFDELCMD < \end{proposition}
%DIFDELCMD < %%%
\DIFdelend %DIF > % \begin{proposition}\label{prop:bigger better}
%DIF > %   Let $\obs = (\meas_1,\dots,\meas_m)^t$, $j \in \{1,\dots,m\}$, $\sigma^2
%DIF > %   > 0$ and $\lambda > 1$. Then $\tar(\obs)$ increases if we use
%DIF > %   $\lambda \meas_j$ in $\obs$ instead of $\meas_j$.
%DIF > % \end{proposition}
\DIFaddbegin \biggerbetter
\DIFaddend \begin{proof} 
...
\DIFdelend \DIFaddbegin \begin{align*}
  ...
  \end{align*} 
  \DIFaddend Since $\postcov$ is positive definite, $\delta \tar(\obs) V > 0$.
\end{proof}
