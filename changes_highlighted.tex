...
setting notation. \reftwo{\sout{At this point, it is useful to record a known result
...
\eqref{eq:optimization} \reftwo{is analytically tractable} and find
...
\begin{align}%\label{eq:old pre manipulations}
  ...
  \begin{split}
    ...
    \obs \fwd \prcov^{1/2}\right ) &= \prcov^{1/2} ( \prcov^{-1} +
...
\begin{align}\label{eq:pre manipulations}
  \begin{split}
    ...
    %
    \det \left (  \fwd \prcov^{1/2}(I + \sigma^{-2} \prcov^{1/2} \fwd ^* \obs^*
    \obs \fwd \prcov^{1/2}) \prcov^{1/2} \fwd^* (\fwd \prcov \fwd^*)^{-1} \right ) \\
    %
    %
    %
    &= \det \left ( (\fwd \prcov \fwd^* + \sigma^{-2}\fwd \prcov
    \fwd^* \obs^*\obs \fwd \prcov \fwd^*) (\fwd \prcov \fwd^*)^{-1}
    \right )\\
    %
    %
    %
    &= \det \left ( ((\fwd \prcov \fwd^*)^{-1} + \sigma^{-2}
    \obs^*\obs ) (\fwd \prcov \fwd^*) \right )\\
  \end{split}
\end{align}
}
...
\reftwo{Results of section \ref{subsub:characterization} specifically,
  the discussion at the end of said section, along with figure
  \ref{figure:clusterization} and eqaution \eqref{eq:true target} show
