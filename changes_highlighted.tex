...
\title[Sensor Clusterization in D-optimal design in infinite
  ...
  Dimensional \reftwo{Linear} Bayesian Inverse Problems}
...
\begin{abstract}
  ...
  \reftwo{linear} inverse problems. We suggest an analytically
...
posterior covariance matrix. In a frequentist setting\reftwo{\sout{s}}, an optimal
...
In the process\reftwo{,} we generalize several lemmas from linear algebra to
...
\begin{figure}
  ...
  \caption{Shown are pointwise posterior standard deviations for
    ...
    1D heat equation with \reftwo{a} homogeneous Dirichlet boundary described in
...
equation in $[0,\pi]$ with \reftwo{a} homogeneous Dirichlet boundary
conditions.
...
(-\Delta)^{-1}$ with \reftwo{a} homogeneous Dirichlet boundary condition. It is
...
\sout{At this point, it is useful to record a known result regarding
  the posterior covariance operator of our inverse problem, namely}
\begin{align}\label{eq:postcov precursor}
  \stkout{\postcov = (\prcov^{-1} + \fwd^* \obs^* \Sigma^{-1} \obs \fwd )^{-1}.}
\end{align}
}



\subsection{Bayesian \reftwo{Linear} Inverse Problems}\label{section:abstract OED}
...
the dynamics given noisy observations of $\fwd \param$. \reftwo{We
  take a zero mean Gaussian prior $\pr$ for $\param$ (otherwise the
  mean can just be subtracted everywhere). Hence, \sout{We model}}
...
Considering the sum of the error terms\reftwo{,} it is easy to see that $\obs
...
it is useful to \reftwo{\sout{rewrite the posterior covariance operator of our
inverse problem from \eqref{eq:postcov precursor}} record a known result
...
\reftwo{(denoted $\tar$ below and throughout the paper)} and is summarized, using our notation,
...
the optimization problem in \eqref{eq:optimization} \reftwo{is
  analytically tractable}.  We find gradients for the new constraints
\reftwo{and record them in \eqref{eq:grad constraints}. Finally, in
  section \ref{subsec:necessary} we use these to formulate the optimal
  design problem as a Lagrange multipliers problem in equation
  \eqref{eq:conditions}.}
...
\subsection{\reftwo{Overview}}
\reftwo{In this section we analyze optimal designs when no model error
  is present. We show how sensor clusterization arises in section
...
\subsection{\reftwo{The Eigenvalue Problem}}\label{subsec:eigenproblem}
...
eigenvectors. \refthree{Below, we use \eqref{eq:postcov} in the second
  equality.} If $\fwd$ is invertible, \refthree{\sout{then we may
    write} the third equality is trivial. Otherwise, we use some
...
\begin{align}\label{eq:mod conditions}
  ...
  \end{split}
\end{align}  
\refthree{\sout{which} This} presents the optimal design problem as a
problem on $\ban$ and not on $\hil$ --- the term $(\fwd \prcov
\fwd^*)^{-1}$ is the prior precision in $\ban$ and \reftwo{\sout{the
    big parentheses enclose} $(\fwd\prcov\fwd^*)^{-1} + \sigma^{-2}
  \obs^* \obs$ is} the posterior precision in
$\ban$. \refthree{\sout{This expression makes sense even if $\fwd$ is
    not invertible --- consider instead $\fwd$ restricted to $\ker
    \fwd^{\perp}$. This does not change the optimal design problem at
    all, since we will never take measurements that have a component
    in $\ker \fwd$ (this can be understood from Corollary
    \ref{cor:zero mod err}).}}
...
\geq k+1$. \refthree{\stkout{By the argument in the paragraph following
    \eqref{eq:mod conditions}:}}

\refthree{
  \begin{align}%\label{eq:old pre manipulations}
    \cancel{
      \begin{split}
        I + \sigma^{-2} \prcov^{1/2} \fwd ^* \obs^*
        \obs \fwd \prcov^{1/2} &= \prcov^{1/2} ( \prcov^{-1} +
        \sigma^{-2} \fwd^*\obs^* \obs \fwd ) \prcov^{1/2} \\
        %
        %
        %
        &=\prcov^{1/2} \fwd^* ( \fwd^{-*}
        \prcov^{-1}\fwd^{-1} + \sigma^{-2} \obs^* \obs ) \fwd \prcov^{1/2}
        (\fwd\prcov\fwd^*)^{-1} \\
        %
        %
        %
        &= \prcov^{1/2} \fwd^* \left ( (\fwd \prcov \fwd^* )^{-1}
        +\sigma^{-2} \obs^* \obs \right ) \fwd \prcov^{1/2}.
      \end{split}
    }
\end{align}
}
...
\reftwo{Results of section \ref{subsec:characterization} specifically,
  the discussion at the end of said section, along with figure
...
$$ \sout{as $\sigma^2 \to 0$, the increase in the design criterion
  gained by taking a measurement identical to a previous one
  approaches zero}. Hence no increase in the design criterion is
