...
evaluations of a continuous function\footnote{Of course, this is
  impossible if, e.g., $\hilo = L^2(\Omega)$ for some $\Omega$. Still,
  this approach is in line with previous analyses
  \cite{Karina,AlexanderianGloorGhattas14} that consider sensor
  placement but formulate the inverse problem on Hilbert spaces.}. Such
...
\begin{restatable}[Characterizing D-optimal designs]{theorem}{main}\label{thm:char}
Denote $\{\lambda_i, \ev_i\}_{i=1}^\infty$ the eigenvalues and
eigenvectors of $\fwd\prcov\fwd^*$, in decreasing eigenvalue order.
Denote $\{\eta_i\}_{i=1}^\infty$ eigenvalues of $\obs^*\obs$, chosen
to maximize $\tar(\obs)$. Denote $m$ number of measurements taken and
$\sigma^2$ observation noise variance. Then:
\begin{enumerate}
\item $\obs^*\obs$ and $\fwd\prcov\fwd^*$ are simultaneously diagonalizable.
\item There exists $k \leq m$ such that $\eta_i = 0$ for $i\geq k+1$.
\item $\sigma^2\lambda_i^{-1} + \eta_i \leq \sigma^2\lambda_{k+1}^{-1}, i=1,\dots,k$.
