...
(e.g.\ pointwise evaluations on $[0,\pi]$). We call this simultaneous
optimization.
...
\begin{equation}\label{eq:def obs_k}
  \obs_k := \sum_{i=1}^k \meas_i\meas^*_i.
\end{equation}
...
This paper is mostly focused with the more general simultaneous
...
all conclusions we make on the simultaneous case easily specialize to
the sequential case by considering the posterior as the next
sequential step's prior.
...
This allow us to arrive at the following:
...
\end{proof}
Note that all but finitely many of these eigenvectors have zero
eigenvalue, since $\obs^*\obs$ is finite-rank. Denote the
corresponding non-zero eigenvalues of $\obs^*\obs$ by
$\{\eta_i\}_{i=1}^{k}$ and let $\eta_i = 0$ for $i \geq k+1$. Using
Sylvester's determinant theorem we get:
...
eigenvector is a lab cylinder (figure \ref{fig:cylinder}). Each
cylinder $i$ is filled, a-priori, with green liquid
$\sigma^2\lambda_i^{-1}$ volume units (this is the prior precision,
weighted by obsevation noise). An optimal design with (e.g.) four
measurements adds blue liquid of four volume units. It does so by
adding each drop to whatever cylinder has the lowest level of liquid
in it (lowest precision). The result of such a procedure is
illustrated in figure \ref{fig:optimal vs not}. The analysis
conducted above is summarized in theorem \ref{thm:char} below.
...
\begin{theorem}[Characterizing D-optimal designs]\label{thm:char}
...
%% We are also used to symmetric optimization problems having
%% symmetric solutions \cite{Waterhouse83}, so symmetric and identical
%% Lagrange multipliers definitely make sense in this context.
...
%% illustrated in figure \ref{fig:clusterization} and
...
\eqref{eq:def obs_k}. The way clusterization can occur is understood
using results of the previous section, specialized to one measurement
each time. By corollary \ref{cor:ame ev}, the first measurement
$\meas_1$ has to be (parallel to) an eigenvector of
$\fwd\prcov\fwd^*$. By theorem \ref{thm:char}, it will be taken in the direction of the
first eigenvector --- $\meas_1 \parallel \ev_1$. As hinted in
section \ref{subsec:seq vs sim}, the posterior becomes the prior for
the next step. Similar reasoning leads to:
\begin{align*}
  \begin{split}
...
\begin{lemma}\label{lemma:free}
  ...
  m$, $m \geq k$. Then we can find $\func_j \in \R^k,j=1,\dots,m$ with
