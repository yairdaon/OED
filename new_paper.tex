...
\cite{ChalonerVerdinelli95}: for a linear model, a D-optimal design
seeks to maximize the expected information gain (KL divergence
\cite{KullbackLeibler51,CoverThomas12}) between posterior and
prior. In finite dimensions, maximizing the expected information gain
...
covariance. However, \cite{ChalonerVerdinelli95} show that the latter
...
``weird'' eigenvalue problem (``weird'' because the operator depends
on its eigenvectors, see \eqref{eq:eigenproblem}). This helps us shed
light into D-optimal designs. We characterize these designs using
theorem \ref{thm:char}. One key and intuituve insight we prove is that
a D-optimal design reduces uncertainties where they are highest
first. More interestig phenomena arise, but they require setting
notation, so these are discussed later.

In the process we generalize several lemmas from linear algebra to
infinite-dimensional settings. A matrix determinant emma and a lemma
due to Lax \cite{lax97} for calculating the derivative of determinants
of certain operators (\ref{lemma:MDL} and \ref{lemma:lax},
respectfully). We also prove some non-trivial lemmas in linear
algebra. One of which (\ref{lemma:free}) gives a decomposition $M =
AA^t$ where $A$ has unit norm columns. Another (\ref{lemma:sim diag})
shows simultaneous diagonizability for the ``weird'' eigenvalue problem
referenced above. We also provide other tools for understanding
D-optimal designs in infinite-dimensional Bayesian inverse problems,
including lemmas for calculating the increase in the design criterion
per observation --- Lemmas \ref{lemma:design increase} and Corollary
...
sequential optimal design. Then we consider simultaneous
designs.
...
(\meas_1,\dots,\meas_{m-1})^t$. Every symbol with a wide hat
$\ \widehat{}\ $ refers to the marked object for $\obsm$. E.g. $\Sigmam =
\Sigma(\obsm)$.
